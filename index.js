...
<div class="artist_form_upper_info_container">
  ...
  `;y.insertAdjacentHTML("beforeend",m)}function G(e){y&&(y.innerHTML=""),p&&(p.innerHTML=""),_("#artist-modal"),fetch(`https://sound-wave.b.goit.study/api/artists/${e}`).then(s=>{if(!s.ok)throw new Error(s.status);return s.json()}).then(s=>U(s)).catch(s=>console.log("Error fetching artist data:",s)),V(e)}async function Q(e){try{return(await R.get(`/artists/${e}/albums`)).data}catch(s){throw console.error("Error fetching artist albums:",s),s}}async function V(e){try{const s=await Q(e);if(!s.albumsList||s.albumsList.length===0){p&&(p.innerHTML="<p>Альбомів не знайдено</p>");return}const a=o=>{const t=Math.floor(Number(o)/1e3),r=Math.floor(t/60),c=t%60;return`${r}:${c.toString().padStart(2,"0")}`},i=`
